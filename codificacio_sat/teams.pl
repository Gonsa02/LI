...
incompatibleWorkers(W1,W2):- maxScore(MS), workerScore(W1, S1), workerScore(W2, S2), W1 < W2, Score is S1+S2, Score > MS.
...
eachWorkerExactlyOneTeam. %S'ha de posar per quan ja no queden workers.

minimTsize:- minSize(MS), team(T), findall(wt(W, T), worker(W), Lits), atLeast(MS,Lits), fail.
minimTsize.

maximTsize:- maxSize(MS), team(T), findall(wt(W, T), worker(W), Lits), atMost(MS,Lits), fail.
maximTsize.

incompatibilites:- incompatibleWorkers(W1, W2), team(T), writeOneClause([-wt(W1, T), -wt(W2,T)]), fail.
incompatibilites.
